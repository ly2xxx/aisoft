...
ai-dev-flow() {
    ...
    case $command in
        ...
        "deploy")
            ...
            ;;
        *)
            echo "Usage: ai-dev-flow {init < /dev/null | code|test|review|docs|deploy} [options]"
            ;;
    esac
}
...
ai_code_gen() {
    ...
    if [[ "$input" =~ ^https?:// ]]; then
        ...
        feature_name=$(basename "$input"  < /dev/null |  sed 's/[^a-zA-Z0-9]/-/g' | tr '[:upper:]' '[:lower:]')
...
generate_from_url() {
    ...
    if command -v gemini &> /dev/null; then
        ...
        echo "URL: $url\nContent:\n$content"  < /dev/null |  gemini "Analyze this web content and provide implementation recommendations for $language. Focus on API integration, data structures, and best practices." > "url_analysis_$(date +%s).md"
...
ai_code_review() {
    ...
    if command -v gemini &> /dev/null; then
        ...
        find ./src -name "*.js" -o -name "*.py" -o -name "*.ts"  < /dev/null |  head -10 | while read file; do
...
ai_git_commit() {
    ...
    local main_branch=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null  < /dev/null |  sed 's@^refs/remotes/origin/@@' || echo "main")
    if [ -z "$main_branch" ]; then
        main_branch="main"
        if \! git show-ref --verify --quiet refs/heads/main; then
            main_branch="master"
